...
from modules.utils.kicad_scan_lib import  load_organized_fp, get_pin_abs_pos_from_organized_lib, load_organized_lib
...
lib_index = load_organized_lib(Path("./export/organized_lib.json"))
...
def rm_extra_slash():
    """
    Remove the extra backslah in some power symbols' values
    Not sure why this happens, likely due to kicad skip library parsing issue.
    """

    schematic_path = sch_filename

    with open(schematic_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Replace the extra backslash with an empty string
...
def update_block_coordinates(pos_x=150, pos_y=100, reference="U1"):
    schematic = load_schematic(sch_filename)
    for component in schematic.symbol:
        if component.Reference.value != reference:
            continue
        coord_list = []
        if isinstance(component.pin, my_skip_lib.sexp.parser.ParsedValue):
            # For symbol wih a single pin, no blocking so far.
            coord_to_block[round(pos_x, 2), round(pos_y, 2)] = {
                "block_x": [],
                "block_y": [],
                "position": "Center"
            }
        ...
        xs = [x for x, y in coord_list]
        ys = [y for x, y in coord_list]
        ...
        for i in range(len(coord_list)):
            ...
            # Block the coordinates for the symbol pin that on the same line with other pins, record its position on the symbol.
            coord_to_block[(round(xi, 2), round(yi, 2))] = {
                "block_x": block_x,
                "block_y": block_y,
                "position": position
            }

def add_power_symbol(symbol_name="GND", reference="#PWR1", pos_x=150, pos_y=100):
    ...
    if reference is None or len(reference) == 0:
        if RAISE_ERR_FLAG:
            raise ValueError("Power symbol reference string can not be empty! It must start with PWR, e.g. #PWR_GND1, #PWR_VCC1, etc.")
        else:
            print("Error: Power symbol reference string can not be empty! It must start with PWR, e.g. #PWR_GND1, #PWR_VCC1, etc.")
            reference = "#PWR1"  # Default reference for power symbols

    # power symbol 
    symbol_lib = "power"
    reference = reference # reference for power symbols must start with PWR
    value = symbol_name

    add_symbol_from_lib(
        sch_filename=sch_filename,
        sym_filename= KICAD_SYMBOL_LIB_PATH + symbol_lib + ".kicad_sym",
        symbol_name=symbol_name,
        lib_name=symbol_lib,
        x=pos_x,
        y=pos_y,
...
def add_RLC_symbol(symbol_name="R", pos_x=150, pos_y=100, reference="R1", value="10K", rotation=0):
    # RLC symbol
    symbol_lib = "Device"
    ...
    if value == "":
        if RAISE_ERR_FLAG:
            raise ValueError("Value must be provided for RLC symbols. Chose a default value, e.g. '10K' for resistors.")
        else:
            print("Error: Value must be provided for RLC symbols. Chose a default value, e.g. '10K' for resistors.")
            value = "0"
    ...
    # move_symbol(reference, 0, 0, rotation=rotation)  # Rotate the symbol to the correct orientation
...
def save_schematic(schematic: my_skip_lib.Schematic, out_path: str):
    ...
    # Bug fix: remove the extra backslash in the symbols' values
    rm_extra_slash()
...
def equal(x1, x2):
    """
    Check if two values are equal within a tolerance of 1.27mm.
    """
    return abs(x1 - x2) < 1.27


def move_symbol(symbol_ref, dx, dy, rotation=0, mirror=None):
    """
    Move a symbol in the schematic by dx, dy and rotation.

    Args:
        symbol_ref (str): The reference of the symbol to move.
    ...
    # print("All symbols:", list(schematic.symbol))
    ...
    x, y, rot = symbol.at.value
    # Set the new location
    symbol.move(align_to_grid(x + dx), align_to_grid(y + dy), rot + rotation)


    # perform mirror operation if needed
    edit_mirror(symbol, mirror)
    
    # Save the schematic
    save_schematic(schematic, sch_filename)


def mirror_symbol(symbol_ref, mirror: str):
    """
    Perform Mirror operation on a symbol in the schematic.
    """
    schematic = load_schematic(sch_filename)
...
def edit_mirror(symbol, mirror: str):
    """
    Perform Mirror operation on a symbol in the schematic.
    """
    if mirror is not None and mirror != "None" and mirror != "none":
        # Set the mirror direction
        if mirror == "X" or mirror == "x":
            mirror_val = 'x'
        elif mirror == "Y" or mirror == "y":
        ...
        if hasattr(symbol.mirror, 'value'):
            ...
            symbol.mirror.value = mirror_val
        else:
            # If not, Need to add mirror attribute to the symbol raw data.
            import sexpdata
            symbol.raw.insert(3, [sexpdata.Symbol('mirror'), sexpdata.Symbol(mirror_val)])
...
def add_new_wire(start_pos: list, end_pos: list, reY_off: bool = False, allow_diagonal: bool = False) -> list | None:
    ...
    # check_box_overlap_error((x1, y1, x2, y2), "Wire", f"[{start_pos[0]:.3f}, {reY(start_pos[1]):.3f}] -> [{end_pos[0]:.3f}, {reY(end_pos[1]):.3f}]", (x1+x2)/2,  (y1+y2)/2)
...
def get_pin_location(symbol_ref: str, pin_name: str):
    ...
    if isinstance(pin_instance, my_skip_lib.eeschema.schematic.symbol.SymbolPin):
        ...
        symbol_pos = pin_instance.parent.at.value[:2]
        symbol_rot = pin_instance.parent.at.value[2] if len(pin_instance.parent.at.value) > 2 else 0.0

        lib_id = pin_instance.parent.lib_id.value  # e.g. "Device:R" or "power:+3.3V"
        if ":" in lib_id:
        ...
        if symbol_lib == "power":
            # For power symbol, directly use symbol position as pin position, because power symbol usually only has one pin at the center.
            pin_pos = symbol_pos
        else:
            pin_pos = get_pin_abs_pos_from_organized_lib(
                org_lib=lib_index,
                lib_name=symbol_lib,
                symbol_name=symbol_name,
                pin_number=pin_name,
                symbol_xy=symbol_pos,
                symbol_rotation_deg=symbol_rot,
            )
            
        # Change pin_pos from tuple to list for later modification.
        pin_pos = list(pin_pos)
    else:
...
def connect_pin_to_label(symbol_ref: str, pin_name: str, label: str,
    ...
    """
    Place a global label and hook it to the given pin. 
    Args:
        symbol_ref (str): The reference of the symbol.
        pin_name (str): The name of the pin.
        label (str): The text of the label.
        x_off (float): The offset in the x direction from the pin position to the label position. Default is 0.0mm.
        y_off (float): The offset in the y direction from the pin position to the label position. Default is 0.0mm. Only one of x_off and y_off can be non-zero.
        label_type (str): The type of the label. Default is "bidirectional". Other options are "input", "output", "Passive" etc.
    """

    assert x_off != 0 or y_off != 0, "Either x_off or y_off must be non-zero to place the label away from the pin."

    if REVERSE_Y_FLAG:
        y_off = -y_off
    # assert x_off == 0 or y_off == 0, "Only one of x_off and y_off can be non-zero."

    try:
        pin_x, pin_y = get_pin_location(symbol_ref, pin_name)
    except Exception:
        # retry with alternate naming convention, e.g. 3  ->  Pin_3
        alt = None
        if pin_name.isdigit():
            alt = f"Pin_{pin_name}"
        elif pin_name.startswith("Pin_") and pin_name[4:].isdigit():
            alt = pin_name[4:]
        if alt is None:
            raise  # nothing left to try – propagate original error
        pin_x, pin_y = get_pin_location(symbol_ref, alt)

    lbl_pos = [round(pin_x + x_off, 2), round(pin_y + y_off, 2)]

    text_orient = "left" if x_off < 0 else "right"  # label text orientation based on x_off
    if x_off == 0:
        text_orient = "top" if y_off > 0 else "bottom"

    # if text_orient not in ["left", "right", "top", "bottom"]:
    #     raise ValueError(f"Invalid text orientation: {text_orient}. Must be one of 'left', 'right', 'top', 'bottom'.")
    
    # if x_off > 0 and text_orient == "right":
    #     raise ValueError("If x_off > 0, text_orient must be 'left' or 'top' or 'bottom' to avoid overlap.")
    # if x_off < 0 and text_orient == "left":
    #     raise ValueError("If x_off < 0, text_orient must be 'right' or 'top' or 'bottom' to avoid overlap.")

    label_loc_list = get_global_label_location(label)

    if label_loc_list == None or lbl_pos not in label_loc_list:
        # If the label is not already placed at the same position, we add a new global label.
        if label_type != "power":
            add_global_label(lbl_pos, label, label_type=label_type, text_orient=text_orient)
        else:
            add_power_symbol(symbol_name=label, reference=label, pos_x=lbl_pos[0], pos_y=lbl_pos[1])

    # Connect the pin to the label position
    connect_two_points([pin_x, pin_y], lbl_pos)
...
def write_out_all_wires():
    ...
    for point in junction_connections:
        # TODO: we can skip this due to modified logic in sch_read.py
...
def example_usage():
    ...
    add_power_symbol(symbol_name="VCC", reference="#PWR1", pos_x=200, pos_y=80)

    add_power_symbol(symbol_name="GND", reference="#PWR2", pos_x=200, pos_y=130)
...
def testing_run_code():
    """
    Test the run_sch_edit_code function.
    """
    ...
    add_RLC_symbol(symbol_name="C", pos_x=250, pos_y=110, reference="C1", value="0.1uF")

    # Place a GND power symbol at (350, 100)
    add_power_symbol(symbol_name="GND", reference="#PWR1", pos_x=290, pos_y=100)
    ...
    add_new_wire([100, r1_pin1[1]], r1_pin1)

    # Connect R1 pin 2 to C1 pin 1
    connect_pins("R1", "2", "C1", "2")
...
if __name__ == "__main__":
    ...
    fps = get_fp(sch_filename="/Users/luoqinpei/Workspace/llm4circuit/dataset/15335_9DoF_Schematic/block_0.kicad_sch")
    print(fps)