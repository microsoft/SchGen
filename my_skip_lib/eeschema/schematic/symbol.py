...
import re
import copy
from my_skip_lib.property import ElementWithPropertiesWrapper
from my_skip_lib.collection import NamedElementCollection
from my_skip_lib.sexp.parser import ParsedValue
from my_skip_lib.at_location import AtValue
from my_skip_lib.eeschema.pin import Pin
...
class SymbolCollection(NamedElementCollection):
    ...
    def reference_matches(self, regex:str):
        ...
        Match symbols with full match
              
        '''
        return list(filter(lambda s: re.fullmatch(regex, s.property.Reference.value), self))
    ...
    def value_matches(self, regex:str):
        return list(filter(lambda s: re.fullmatch(regex, s.property.Value.value), self))
...
class SymbolPin(Pin):
    ...
    def location(self):
        ...
        rel_at.value[1] = -1 * rel_at.y  # note the - !!coords in lib editor are flipped kinda
